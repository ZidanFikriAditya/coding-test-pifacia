...
const form = ref<{
    ...
    participant_id: {
        value: props.payment?.participant_id || null,
        preview: null,
        error: '',
        required: true,
    },
    file_path: {
        value: null,
        preview: props.payment?.file_path || null,
        error: '',
        required: false,
    },
    uploaded_at: {
        value: moment(props.payment?.uploaded_at).format('YYYY-MM-DDTHH:mm:ss') || null,
        preview: null,
        error: '',
        required: true,
    },
...
>(
    props.payment.metadata?.map((item) => ({
        value: item,
        error: null,
    })) || [
        {
...
const handleSave = () => {
    ...
    metaDatas.value.forEach((item, index) => {
        formData.append('metadata[]', item.value);
    });

    axios
        .post(basePath + `/${props.payment.id}`, formData)
        .then((response) => {
            ...
            if (error.response.status === 422) {
                ...
                Object.keys(errors).forEach((key) => {
                    ...
                    metaDatas.value.forEach((_, index) => {
                        if (errors[`metadata.${index}`]) {
                            metaDatas.value[index] = {
                                ...metaDatas.value[index],
                                error: errors[`metadata.${index}`][0] || '',
                            };
                        }
                    });
                });
...
const getLink = computed(() => form.value?.file_path?.preview);
</script>
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="container mx-auto px-4 py-8">
            ...
            <div class="grid grid-cols-4 gap-4">
                <form class="col-span-3" @submit.prevent="handleSave">
                    <div class="mt-5 grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <Input type="file" name="file_path" id="file_path" placeholder="Enter file here..." @input="handleChange" />
                        ...
                        <div class="col-span-2">
                            <div class="mb-3 grid grid-cols-4 items-end gap-3" v-for="(metadata, index) in metaDatas" :key="`extra_data${index}`">
                                <div class="col-span-3">
                                    ...
                                    <Input
                                        ...
                                        @update:model-value="(value) => (metaDatas[index] = { ...metaDatas[index], value: value.toString() })"
                                ...
                                <div class="col-span-1 inline-flex items-center gap-2">
                                    ...
                                    <Button type="button" v-if="metaDatas.length > 1" @click="metaDatas.splice(index, 1)" variant="destructive"
                                        ><Trash />
                                    </Button>
                                </div>
                            </div>
                ...
                <div>
                    <AuditTrail :slug="'payment'" :id="props.payment.id" />
                </div>
            </div>
        </div>
    </AppLayout>
